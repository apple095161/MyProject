  ...
  <div>
    ...
    <div class="bgs">
      <div class="logined">
        ...
        <div class="logined-title" v-if="index == 1">IG LIFE會員登入</div>
        <div class="logined-title" v-else>IG LIFE會員註冊</div>
        ...
        <div class="body-form" v-else>
          ...
          <div class="form-set">
            <label for="reg-email" class="form-label">帳號*</label>
            <input
              type="text"
              class="form-input"
              placeholder="請輸入電子郵件"
    ...
    login(key) {
      ...
      const result = (response) => {
        if (response.data.isSuccess == true) {
          ...
          } else if (JSON.parse(userString).roles == '2') {
            localStorage.setItem('User', userString);
            localStorage.setItem('Jwt', response.data.data);
            this.$router.push({
              name: 'dashboard',
