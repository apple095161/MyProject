...
<!-- 
      ...
      <b-table
        ...
        v-if="!itemshow"
      >
        <template #cell(pddesc)="row" class="test123">
          <editor
            :resizable-content="true"
            :resizable-toolbar="true"
            rows="20"
            cols="30"
            v-model="row.item.pddesc"
        ...
        <template v-slot:row-details="row">
          <b-card>
            <div>
              <h5 class="mb-0"><strong>商品尺寸</strong></h5>
              <div>{{ row.item.pdsize }}</div>
            </div>
            <div>
              <h5 class="mb-0"><strong>商品簡述</strong></h5>
              <div>{{ row.item.pdshort }}</div>
            </div>
            <div>
              <h5 class="mb-0"><strong>商品描述</strong></h5>
              <div v-html="row.item.pddesc"></div>
            </div>
          </b-card>
        </template>

        <template v-slot:cell(pdshile)="row">
          {{ $DateFormatNotTime(row.item.pdshile) }}
        </template>
        <template v-slot:cell(pdprice)="row">
          ${{ $addcomma(row.item.pdprice) }}
        </template>
        <template v-slot:cell(pdavailable)="row">
          <div class="text-center">
            <i
              class="far fa-check-circle text-success mr-4"
              v-if="row.item.pdavailable"
              style="font-size: 22px"
            ></i>
            <i
              class="far fa-times-circle text-danger mr-4"
              style="font-size: 22px"
              v-else
            ></i>
          </div>
...
export default {
  ...
  methods: {
    getData() {
      const url = '/api/v1/ProductApi/All';
